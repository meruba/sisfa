...
class ItemHospitalizacion < ActiveRecord::Base
  ...
  def anular_item(razon)
    ...
    add_liquidacion(self)
    self.save
  end

#methods for anulado item
  def rollback_item
...
#method liquidacion anulado
  def add_liquidacion(item)
    subtotal = 0
    subtotal_12 = 0
    iva = 0
    ingreso = IngresoProducto.find item.ingreso_producto_id
    costo = (ingreso.producto.precio_compra * item.cantidad)
    #valores para el comprobante
    if ingreso.producto.hasiva == true
    ...
    Liquidacion.add_item_hospitalizacion(item, item.hospitalizacion,iva_item,subtotal_item,subtotal_12_item,total_item,costo)
  end
