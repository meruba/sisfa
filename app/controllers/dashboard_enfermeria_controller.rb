class DashboardEnfermeriaController < ApplicationController
  ...
  before_filter :is_admin_or_enfermera_enfermeria
	def index
		...
		@camas = HospitalizacionRegistro.includes([paciente: [:cliente]],[asignacion_cama: [:cama]]).where(:alta_enfermeria => false).where.not(:asignacion_camas => { :hospitalizacion_registro_id => nil }).references([paciente: [:cliente]],[asignacion_cama: [:cama]])
	end
end
